...
const CountrySearchBar = () => {
  ...
  const [fetchCountries, { loading, error, data }] =
    useLazyQuery(searchCountries);
  ...
  const handleInputChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (isSearchGroupText) {
      ...
      if (matches != null) {
        //Since Query group is not supported, I am only taking the search part here and querying it.
        inputText = matches[1];
      }
    }
  ...
  return (
    <InputGroup className="mb-3">
      <Form.Control
        ...
        placeholder="[Exp] search:Turkey group:size"
      />
    </InputGroup>
  );
};
